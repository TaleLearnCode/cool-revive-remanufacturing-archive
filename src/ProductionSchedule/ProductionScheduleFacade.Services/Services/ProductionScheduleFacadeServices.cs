...
public class ProductionScheduleFacadeServices(TableClient tableClient)
{
	...
	public async Task<IResponse> GetNextCoreAsync(string podId, string date, string instance)
	{
		try
		{
			...
			List<ProductionScheduleTableEntity> productionSchedule = [.. _tableClient.Query<ProductionScheduleTableEntity>(x => x.PartitionKey == $"{podId}_{date}" && x.Status == "Scheduled")];
			productionSchedule.Sort((x, y) => x.Sequence.CompareTo(y.Sequence));

			ProductionScheduleTableEntity? nextCore = productionSchedule.FirstOrDefault();
